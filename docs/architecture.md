...
- Anyone can ignore themselves in a channel with `ignore me`; the entry's `added_by` is their own id. The bot still hears `unignore me` from them, and only from them, and only while every ignore that applies to them is one they set. An ignore a moderator or an admin set stays until one of them lifts it.
