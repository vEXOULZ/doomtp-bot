...
- **Built-ins** only get what their spec lists in `reads` and `writes`. `!var` is the exception, because it's a built-in acting *as the typed expression* and follows the Typed column.
