...
3. [x] Implement token persistence hooks backed by `oauth_tokens` in SQLite. *(`twitch/tokens.py`; refreshes are saved through `event_token_refreshed`. The table moved to the `bot` schema in Postgres with ADR-0014; the hooks did not change.)*
