...
| **moderator** | The broadcaster mods the bot. The bot token adds `moderator:read:followers`, `moderator:manage:banned_users`, `moderator:manage:chat_messages`, `moderator:manage:shoutouts` (added 2026-09-23 for `!shoutout`; a bot signed in before then signs in again to send the card) and the moderator read scopes for `channel.moderate`. | Adds `channel.follow`, `channel.moderate` (who and why for mod actions), timeout, ban and delete actions, higher send limits, and exemption from slow and follower modes |
...
3. [x] Build the CapabilityProbe, the `channels.capabilities` column, and feature `requires` declarations. *Built 2026-09-19: `core/capabilities.py` probes at join and hourly by attempting the moderator-only `channel.follow` subscription. `CommandSpec.requires` was already checked against `channels.capabilities` by the policy gate; trigger types now name the capability they need when it is missing. Not yet exercised against a live modded channel.*
4. [x] Build the Helix `Get Streams` poller for basic-tier stream status. *Built 2026-09-19: `core/streams.py`, one batched request a minute, feeding `only_live` and the `stream_online`/`stream_offline` triggers.*
5. [x] Build the `/auth/connect` broadcaster flow and per-broadcaster token storage. *Built 2026-09-19: `twitch/auth.py` serves both flows from one callback, told apart by the `state`; tokens are stored as `broadcaster:<user_id>` and restored at startup; granted scopes become capabilities through `granted_by` and `CapabilityProbe.grant`, and the redemption and cheer subscriptions are created with the broadcaster's token. A grant taken back on Twitch is noticed when a subscription comes back 401 or 403: the token is dropped and `revoke_full` puts the channel back to what the bot earned by itself. Not yet exercised against a live channel.*
