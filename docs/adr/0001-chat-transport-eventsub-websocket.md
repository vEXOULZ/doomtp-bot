...
5. [x] Build a reconnect test against the Twitch CLI mock EventSub server. *Done 2026-09-20: `tests/chat/test_eventsub_mock_server.py` runs the bot's own client and handlers against `twitch event websocket start-server` — welcome, session id, an event off the wire into the dispatcher's sink, then `twitch event websocket reconnect` and the same again on the session that replaces it. It starts its own server on a free port and skips itself where the CLI isn't installed; CI installs it.*
6. [x] Add `message_id` dedupe (LRU) in the adapter. *(`TwitchService.emit`, 2000 ids)*
