...
2. [x] Warning strings are in the chat replies for link, publish and grant; `!cc info` shows "changed since vN"; the API and web UI show and set what a channel published. *Finished 2026-09-20: a run now records the version of each publication it reached, which is what "changed since" compares against — until today nothing called `touch_run`, so it never moved. With `cc_edit_notice` on, the channel also hears it once in chat, right after the reply: "heads up: hi changed since v2 — @alice edited it (now v3)". Only this channel's own publications: a command reached globally or through a pack has no row here to remember it by.*
3. [x] Parameter declaration (`!cc param`), validation and generated usage text. *Built 2026-09-19: `customcmds/params.py` turns the stored declarations into the same `Param` objects built-in specs use, so `spec_for` gives a custom command real usage text, `!help` and `!cc info` describe it, and the executor binds and validates arguments the same way for both. A body with no declarations still takes free arguments through `{arg.*}`.*
4. [x] Bodies are parsed at save time and expanded in preflight, with depth and cycle checks (`E_CC_DEPTH`, `E_CC_CYCLE`). *Finished 2026-09-20: the channel's filter now runs over everything `cc` stores — the body and command name on `add`, the body on `edit`, the summary on `describe`, a parameter's name and description, a pack's name and summary, a personal alias on `link`, and the name a command is published under. Anything the filter would change is refused with the patterns it matched, because all of it is read back out later as a name, a usage line or a reply.*
5. [x] Add API endpoints `/api/v1/custom-commands` and `/api/v1/channels/{login}/publications` for the public page. *Built 2026-09-19, both public: `/custom-commands` lists what is published globally, or one owner's shared commands with `?owner=`; a private command is never listed.*
6. [x] `!cc run <id>` to bypass resolution, and `!help` listing the custom commands a caller can run here. *Both built 2026-09-19. `cc run` takes the owner's own commands and any shared one, binds declared params, runs the body as the invoker with the owner as publisher, and is refused outside Line context so a body can't recurse around preflight's depth checks.*
