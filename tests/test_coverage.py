...
async def _channel(dbs: Databases, channel_id: str, login: str, *, backfill: bool) -> None:
    await dbs.bot.execute(
        ...
        " VALUES (?, ?, ?, ?, ?)",
        (channel_id, login, int(backfill), NOW, NOW),
    )
    await dbs.bot.commit()


async def _session(dbs: Databases, channel_id: str, started: int, ended: int | None, why: str) -> None:
    await dbs.chatlog.execute(
        "INSERT INTO log_sessions (channel_id, started_at, ended_at, end_reason) VALUES (?, ?, ?, ?)",
        (channel_id, started, ended, why),
    )
    await dbs.chatlog.commit()


async def test_it_names_the_gaps_backfill_never_closed(tmp_path: Path, dbs: Databases) -> None:
    ...
    lines, open_gaps = report(tmp_path, recent_days=7)
...
async def test_gaps_are_counted_not_listed_where_backfill_is_off(tmp_path: Path, dbs: Databases) -> None:
...
async def test_old_gaps_and_silent_channels_stay_out_of_the_way(tmp_path: Path, dbs: Databases) -> None:
